...
for x in range(size[0]):
    for y in range(size[1]):
        ...
        for level in range(-levels, levels+1):
            ...
            if x == 0 and level-1 >= -levels:
                thisNeighbors.append((1, 2, level-1))
            if y == 0 and level-1 >= -levels:
                thisNeighbors.append((2, 1, level-1))
            if x == size[0]-1 and level-1 >= -levels:
                thisNeighbors.append((3, 2, level-1))
            if y == size[1]-1 and level-1 >= -levels:
                thisNeighbors.append((2, 3, level-1))

            if (x, y) == (1, 2) and level+1 <= levels:
                thisNeighbors += [(0, yi, level+1) for yi in range(size[1])]
            if (x, y) == (2, 1) and level+1 <= levels:
                thisNeighbors += [(xi, 0, level+1) for xi in range(size[0])]
            if (x, y) == (3, 2) and level+1 <= levels:
                thisNeighbors += [(size[0]-1, yi, level+1) for yi in range(size[1])]
            if (x, y) == (2, 3) and level+1 <= levels:
                thisNeighbors += [(xi, size[1]-1, level+1) for xi in range(size[0])]
